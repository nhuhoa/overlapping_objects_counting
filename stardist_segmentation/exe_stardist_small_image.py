...
bksize=1024,min_overlp=64):
    model = StarDist2D.from_pretrained(model_name)
    
    ## TO DO: get dimension of image, in order to select the correct prediction
    ## Large image
    #labels, polys = model.predict_instances_big(obs_img, axes='YX', block_size=bksize, min_overlap=min_overlp, context=94)
    
    ## Small image
    labels, polys = model.predict_instances(obs_img, axes='YX')
...
#     print(args['datatag'])
    ...
    trained_model = '2D_paper_dsb2018' # best model for object counting issue here
    run_stardist(args['input_dir'], args['image_fn'], args['output_dir'], args['trained_model'], args['output_fn'], args['is_normalized'])
